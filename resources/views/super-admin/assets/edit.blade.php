<x-layouts.app>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            <form action="{{ route('super-admin.assets.update', $asset) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            ...
                            <option value="computer" {{ old('type', $asset->type) == 'computer' ? 'selected' : '' }}>Computer</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="assigned_date" id="assigned_date"
                               value="{{ old('assigned_date', $asset->assigned_date?->format('Y-m-d')) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        @error('assigned_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('description', $asset->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('department_id').addEventListener('change', function() {
                ...
                const currentUser = '{{ $asset->assigned_to }}';
            ...
            // Initialize assigned date requirement based on initial user assignment
